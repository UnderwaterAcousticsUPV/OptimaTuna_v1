...
df <- df_byjal[[i]]
df$subt[df$subt == 0] <- NA
p_j12 <- ggplot(df, aes(x = fecha, y = alim_ind, fill = gras_porc_macroAve)) +
  geom_bar(stat = 'identity') +
  scale_fill_gradient2(mid='navy', high="limegreen", name="Grasa [%]") +
  scale_x_date(date_breaks = "1 month") +
  theme_grey() +
  labs(title = paste0("Alimentación JAL12: campaña 2021")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = " ") + labs(y = "Alimento [Kg/ind/d]") +
  geom_segment(aes(x = fecha, xend = fecha, y = 0, yend = subt/15), col="red", size=0.6) +
  geom_point(data=df, aes(x = fecha, y = subt/15), col="red", size=4) +
...
df <- df_byjal[[i]]
df$subt[df$subt == 0] <- NA
p_j16 <- ggplot(df, aes(x = fecha, y = alim_ind, fill = gras_porc_macroAve)) +
  geom_bar(stat = 'identity') +
  scale_fill_gradient2(mid='navy', high="limegreen", name="Grasa [%]") +
  scale_x_date(date_breaks = "1 month") +
  theme_grey() +
  labs(title = paste0("Alimentación JAL16: campaña 2021")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = " ") + labs(y = "Alimento [Kg/ind/d]") +
  geom_segment(aes(x = fecha, xend = fecha, y = 0, yend = subt/15), col="red", size=0.6) +
  geom_point(data=df, aes(x = fecha, y = subt/15), col="red", size=4) +
